...
class GreedySimplePolicy:
    """
    Lightweight greedy baseline.
    ...
    3) Scheduling:
       choose execution UAV by a simple greedy score combining
...
def generate_greedy_simple_high_action(
    state: Dict[str, Any],
    access_assoc: np.ndarray,
    seed: Optional[int] = None,
) -> Dict[str, np.ndarray]:
    policy = GreedySimplePolicy(seed=seed)
    return policy.act(
        state=state,
