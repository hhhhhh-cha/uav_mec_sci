...
OUT_ROOT="results/sensitivity_v8/refine_n"
LOG_DIR="logs/sensitivity_v8_refine_n"
...
do
  ...
  do
    ...
    python3 ./eval/run_main_comparison_sci_v8.py \
      ...
      --episode-length 20 \
      --model-refine-max-tasks "${NREF}" \
      --model-refine-ratio 1 \
      --model-refine-sched 1 \
      --no-greedy \
      --no-random \
      --out "${OUT_ROOT}/refine${NREF}/trainseed${TRAIN_SEED}_eval10" \
      2>&1 | tee "${LOG_DIR}/refine${NREF}_trainseed${TRAIN_SEED}.log"
  done
done
