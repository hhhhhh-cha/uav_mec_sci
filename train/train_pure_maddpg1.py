...
def main():
    ...
    # Log / save dirs
    # -------------------------------------------------
    run_name = "pure_maddpg_seed72_run1"
    ...
    for ep in range(num_episodes):
        ...
        avg_critic_loss_value = (
            episode_critic_loss / update_count if update_count > 0 else np.nan
        )
        ...
        with open(train_log_path, "a", newline="", encoding="utf-8") as f:
            ...
            writer.writerow([
                ep,
                episode_reward,
                episode_actor_policy_loss / max(step_count, 1),
                avg_critic_loss_value,
                step_count,
                len(buffer),
            ])
    ...
    print("\nPure MADDPG training finished successfully.")
    print("No checkpoints were saved, so original checkpoint files were not overwritten.")
    print("Logs:")
    print(f"  {train_log_path}")
    print(f"  {eval_log_path}")
