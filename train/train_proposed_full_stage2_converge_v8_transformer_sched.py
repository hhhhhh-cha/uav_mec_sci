...
# Stage-2/3 收敛训练版 v8（Transformer-guided scheduling + refined ratio/schedule distillation + one-step model refinement）：
...
#   9) 新增 refined_ratio_bc_loss：把 one-step refine 后的低 cost offloading ratio 蒸馏回 Transformer ratio head
#  10) 降低 Stage-1 ratio anchor / neighbor collaboration / move-schedule anchor，增强 critic-guided 更新
...
def policy_eval_mode(policy):
    modules = [
        ...
        getattr(policy, "schedule_head", None),
    ]
...
def load_policy_weights_inplace(dst_policy, src_policy):
    ...
    """
    for name in ["actor_net", "encoder", "fusion_net", "ratio_head", "schedule_head"]:
...
def soft_update_policy_v8(target_policy, source_policy, tau: float = 0.005):
    """Soft-update all v8 policy-side modules, including schedule_head.

    The helper imported from Stage-1 does not know about schedule_head.
    For v8 Transformer-scheduling training, schedule_head must be updated in
    the target policy; otherwise TD targets are computed with a stale random
    scheduling head.
    """
    for name in ["actor_net", "encoder", "fusion_net", "ratio_head", "schedule_head"]:
        target_module = getattr(target_policy, name, None)
        source_module = getattr(source_policy, name, None)
        if target_module is not None and source_module is not None:
            soft_update(target_module, source_module, tau=tau)
...
# =========================================================
# v8: differentiable Transformer-guided ratio + scheduling branch
# =========================================================
def forward_v8_highlevel_branches(policy, state: Dict[str, Any], access_assoc: np.ndarray, use_hard_min: bool = False):
    """Forward encoder/fusion/ratio_head/schedule_head with gradients.

    Returns:
        ratio_pred: [K]
        schedule_logits: [K, Nc]
        schedule_mask: [K, Nc]
        candidate_j: [K, Nc] int64, -1 for padding
        sched_beta_soft: [1, K, M, M]
    """
    if getattr(policy, "schedule_head", None) is None:
        raise RuntimeError("policy.schedule_head is None. Use build_default_proposed_policy_v8(...).")
    M = int(state["M"])
    K = int(state["K"])
    ...
    task_feat_batches = []
    uav_feat_batches = []
    candidate_j_np = -np.ones((K, Nc), dtype=np.int64)
    ...
    for k in range(K):
        ...
        for n, j in enumerate(legal_js):
            candidate_j_np[k, n] = int(j)

        token_batches.append(tokens)
        mask_batches.append(mask)
        task_feat_batches.append(policy._normalize_task_feature(state, k))
        uav_feat_batches.append(policy._normalize_uav_feature(state, access_m))
    ...
    candidate_j_t = torch.tensor(candidate_j_np, dtype=torch.long, device=policy.device)

    ratio_prior_np = policy._compute_ratio_prior(state, access_assoc)
    prior_tensor = torch.tensor(ratio_prior_np, dtype=torch.float32, device=policy.device).unsqueeze(-1)
    ...
    if use_hard_min:
        ratio_pred = torch.clamp(ratio_pred, min=policy.ratio_floor, max=policy.ratio_ceiling)
    else:
        ratio_pred = torch.clamp(ratio_pred, min=1e-4, max=policy.ratio_ceiling)

    schedule_logits = policy.schedule_head(
    ...
    # Explicitly mask invalid / padded candidates before softmax.
    # This ensures the differentiable scheduling distribution is normalized
    # only over legal candidate UAVs.
    schedule_logits_masked = schedule_logits.masked_fill(mask_tensor < 0.5, -1e9)
    schedule_prob = torch.softmax(schedule_logits_masked, dim=-1)
    ...
    #     uav_feat=uav_feat_tensor,
    # )
    # schedule_prob = torch.softmax(schedule_logits, dim=-1)

    sched_beta_soft = torch.zeros((1, K, M, M), dtype=torch.float32, device=policy.device)
    for k in range(K):
        access_m = int(np.argmax(access_assoc[:, k]))
        for n in range(Nc):
            j = int(candidate_j_np[k, n])
            if j >= 0:
                sched_beta_soft[:, k, access_m, j] = schedule_prob[k, n]

    # return ratio_pred, schedule_logits, mask_tensor, candidate_j_t, sched_beta_soft
    return ratio_pred, schedule_logits_masked, mask_tensor, candidate_j_t, sched_beta_soft

def build_surrogate_full_action_tensor_v8(
    state: Dict[str, Any],
    raw_action_pred: torch.Tensor,
    ratio_pred: torch.Tensor,
    sched_beta_soft: torch.Tensor,
    bandwidth_alloc_np: np.ndarray,
    cpu_alloc_np: np.ndarray,
) -> torch.Tensor:
    """Differentiable full action for v8: mobility from actor, ratio/schedule from Transformer branches."""
    device = raw_action_pred.device
    M = int(state["M"])
    K = int(state["K"])
    max_speed = float(state["max_speed"])
    ...
    return torch.cat(
        [move_dist, move_angle, offload_ratio, sched_beta_soft.reshape(1, -1), bandwidth_alloc, cpu_alloc],
        dim=1,
    )


def refined_schedule_distillation_loss(
    schedule_logits: torch.Tensor,
    schedule_mask: torch.Tensor,
    candidate_j: torch.Tensor,
    refined_sched_beta_np: np.ndarray,
    access_assoc: np.ndarray,
):
    """Cross-entropy from refined hard scheduling beta to Transformer candidate logits."""
    schedule_logits = schedule_logits.masked_fill(schedule_mask < 0.5, -1e9)

    K, Nc = schedule_logits.shape
    device = schedule_logits.device
    refined_sched = np.asarray(refined_sched_beta_np, dtype=np.float32)
    target_indices = []
    valid_rows = []
    correct = []
    for k in range(K):
        access_m = int(np.argmax(access_assoc[:, k]))
        chosen = np.where(refined_sched[k, access_m, :] > 0.5)[0]
        if len(chosen) == 0:
            continue
        target_j = int(chosen[0])
        row_candidates = candidate_j[k].detach().cpu().numpy().astype(np.int64)
        matches = np.where(row_candidates == target_j)[0]
        if len(matches) == 0:
            continue
        target_n = int(matches[0])
        if float(schedule_mask[k, target_n].detach().cpu()) < 0.5:
            continue
        target_indices.append(target_n)
        valid_rows.append(k)

        pred_n = int(torch.argmax(schedule_logits[k]).detach().cpu().item())
        pred_j = int(row_candidates[pred_n]) if pred_n < len(row_candidates) else -1
        correct.append(1.0 if pred_j == target_j else 0.0)

    if len(valid_rows) == 0:
        z = torch.zeros((), dtype=schedule_logits.dtype, device=device)
        return z, z, z

    row_t = torch.tensor(valid_rows, dtype=torch.long, device=device)
    target_t = torch.tensor(target_indices, dtype=torch.long, device=device)
    loss = F.cross_entropy(schedule_logits[row_t], target_t)
    acc = torch.tensor(float(np.mean(correct)) if len(correct) > 0 else 0.0, dtype=schedule_logits.dtype, device=device)

    prob = torch.softmax(schedule_logits[row_t], dim=-1)
    entropy = -(prob * torch.log(torch.clamp(prob, min=1e-8))).sum(dim=-1).mean()
    return loss, acc, entropy

def compute_v8_schedule_neighbor_collaboration_loss(
    schedule_logits: torch.Tensor,
    schedule_mask: torch.Tensor,
    candidate_j: torch.Tensor,
    state: Dict[str, Any],
    ...
    """
    v8 scheduling anti-collapse regularization.

    Unlike the old compute_neighbor_collaboration_loss(...), this one acts on
    TransformerScheduleHead logits, not on the unused MLP scheduling branch.
    It encourages non-local legal neighbor candidates to receive enough
    probability mass when they exist.
    """
    device = schedule_logits.device
    dtype = schedule_logits.dtype

    K, Nc = schedule_logits.shape

    logits = schedule_logits.masked_fill(schedule_mask < 0.5, -1e9)
    ...
    for k in range(K):
        ...
        cand = candidate_j[k]                      # [Nc]
        valid = (cand >= 0) & (schedule_mask[k] > 0.5)
        neighbor_valid = valid & (cand != access_m)

        if torch.sum(neighbor_valid).item() <= 0:
            continue

        neighbor_prob = prob[k][neighbor_valid].sum()
        ...
        losses.append(torch.relu(target - neighbor_prob))
...
def compute_v8_schedule_local_dominance_margin_loss(
    schedule_logits: torch.Tensor,
    schedule_mask: torch.Tensor,
    candidate_j: torch.Tensor,
    state: Dict[str, Any],
    ...
    """
    v8 local-dominance margin loss.

    This penalizes the local candidate logit when it is excessively larger than
    the best legal neighbor candidate logit. It acts directly on
    TransformerScheduleHead logits.
    """
    device = schedule_logits.device
    dtype = schedule_logits.dtype
    ...
    for k in range(K):
        ...
        local_valid = valid & (cand == access_m)
        neighbor_valid = valid & (cand != access_m)

        if torch.sum(local_valid).item() <= 0:
            continue
        if torch.sum(neighbor_valid).item() <= 0:
            continue

        local_score = logits[k][local_valid].max()
        best_neighbor_score = logits[k][neighbor_valid].max()

        losses.append(torch.relu(local_score - best_neighbor_score - margin))
...
def evaluate_full_policy_rollout_with_extra_constraints(env, policy, seed: int):
    ...
    refine_max_tasks = _env_int("MODEL_REFINE_MAX_TASKS", 16)
...
def _temporary_env(updates: Dict[str, str]):
    """Context manager that temporarily overrides os.environ values."""
    @contextmanager
    def _ctx():
        old_values = {}
...
def evaluate_policy_with_refine_override(
    env,
    policy,
    seed: int,
    enable_refine: bool,
    ...
    """Evaluate with an explicit refine mode, independent of global training env vars."""
    with _temporary_env(
        {
            "MODEL_REFINE_EVAL": "1" if enable_refine else "0",
            "MODEL_REFINE_MAX_TASKS": str(int(max_tasks)),
            "MODEL_REFINE_RATIO": "1" if ratio_search else "0",
            "MODEL_REFINE_SCHED": "1" if schedule_search else "0",
        }
    ):
        return evaluate_full_policy_rollout_with_extra_constraints(env=env, policy=policy, seed=seed)


# def save_policy_components(policy, ckpt_prefix: str, tag: str) -> None:
#     """Save actor/Transformer/fusion/ratio-head under a prefix tag."""
#     os.makedirs("checkpoints", exist_ok=True)
#     out_prefix = f"{ckpt_prefix}_{tag}"
#     if getattr(policy, "actor_net", None) is not None:
#         torch.save(policy.actor_net.state_dict(), f"checkpoints/{out_prefix}_best_actor.pth")
#     if getattr(policy, "encoder", None) is not None:
#         torch.save(policy.encoder.state_dict(), f"checkpoints/{out_prefix}_best_encoder.pth")
#     if getattr(policy, "fusion_net", None) is not None:
#         torch.save(policy.fusion_net.state_dict(), f"checkpoints/{out_prefix}_best_fusion.pth")
#     if getattr(policy, "ratio_head", None) is not None:
#         torch.save(policy.ratio_head.state_dict(), f"checkpoints/{out_prefix}_best_ratio_head.pth")
#     if getattr(policy, "schedule_head", None) is not None:
#         torch.save(policy.schedule_head.state_dict(), f"checkpoints/{out_prefix}_best_schedule_head.pth")

def save_policy_components(policy, ckpt_prefix: str, tag: str) -> None:
    """
    Save v8 policy-side components with clear naming.

    Naming rule:
        tag="best":
            checkpoints/{ckpt_prefix}_best_actor.pth
            checkpoints/{ckpt_prefix}_best_encoder.pth
            checkpoints/{ckpt_prefix}_best_fusion.pth
            checkpoints/{ckpt_prefix}_best_ratio_head.pth
            checkpoints/{ckpt_prefix}_best_schedule_head.pth

        tag="final":
            checkpoints/{ckpt_prefix}_final_actor.pth
            checkpoints/{ckpt_prefix}_final_encoder.pth
            checkpoints/{ckpt_prefix}_final_fusion.pth
            checkpoints/{ckpt_prefix}_final_ratio_head.pth
            checkpoints/{ckpt_prefix}_final_schedule_head.pth
    """
    os.makedirs("checkpoints", exist_ok=True)

    tag = "best" if tag in ("", None, "best") else str(tag)

    def save_module(module, suffix: str) -> None:
        if module is not None:
            torch.save(
                module.state_dict(),
                f"checkpoints/{ckpt_prefix}_{tag}_{suffix}.pth",
            )

    save_module(getattr(policy, "actor_net", None), "actor")
    save_module(getattr(policy, "encoder", None), "encoder")
    save_module(getattr(policy, "fusion_net", None), "fusion")
    save_module(getattr(policy, "ratio_head", None), "ratio_head")
    save_module(getattr(policy, "schedule_head", None), "schedule_head")
...
def save_training_state(
    ...
    """
    Save full training state for reproducibility / resume / audit.

    This file is not used by the normal evaluation script, but it is important
    for SCI-level reproducibility and for avoiding loss of all parameters if
    a later diagnostic step fails.
    """
    os.makedirs(os.path.dirname(path), exist_ok=True)

    def module_state(obj, name: str):
    ...
    payload = {
        "meta": dict(meta),
        "policy": {
            "actor_net": module_state(policy, "actor_net"),
            "encoder": module_state(policy, "encoder"),
            "fusion_net": module_state(policy, "fusion_net"),
            "ratio_head": module_state(policy, "ratio_head"),
            "schedule_head": module_state(policy, "schedule_head"),
        },
        "target_policy": {
            "actor_net": module_state(target_policy, "actor_net"),
            "encoder": module_state(target_policy, "encoder"),
            "fusion_net": module_state(target_policy, "fusion_net"),
            "ratio_head": module_state(target_policy, "ratio_head"),
            "schedule_head": module_state(target_policy, "schedule_head"),
        },
...
def main():
    ...
    # The encoder then keeps only input projection, layer normalization, and
    # masked mean pooling, so the only removed component is Transformer
    # self-attention/FFN interaction modeling.
    ...
    for _module in [
        ...
        getattr(stage1_reference_policy, "schedule_head", None),
    ]:
    ...
    if getattr(policy, "schedule_head", None) is not None:
        schedule_params += list(policy.schedule_head.parameters())

    learnable_params = actor_params + ratio_branch_params + schedule_params
    ...
    actor_opt = optim.Adam(
        [
            ...
            {"params": schedule_params, "lr": _env_float("SCHEDULE_BRANCH_LR", 5e-5)},
        ]
    )
    ...
    actor_policy_coef_start = _env_float("ACTOR_POLICY_COEF_START", 0.0003)
    actor_policy_coef_end = _env_float("ACTOR_POLICY_COEF_END", 0.0012)
    ...
    actor_move_sched_bc_coef_start = _env_float("ACTOR_MOVE_SCHED_BC_COEF_START", 1.00)
    actor_move_sched_bc_coef_end = _env_float("ACTOR_MOVE_SCHED_BC_COEF_END", 0.30)

    ratio_bc_coef_start = _env_float("RATIO_BC_COEF_START", 0.020)
    ratio_bc_coef_end = _env_float("RATIO_BC_COEF_END", 0.000)
    ...
    ratio_reg_coef_start = _env_float("RATIO_REG_COEF_START", 0.002)
    ratio_reg_coef_end = _env_float("RATIO_REG_COEF_END", 0.0005)
    ratio_floor_target = _env_float("RATIO_FLOOR_TARGET", 0.05)
    ratio_mean_target = _env_float("RATIO_MEAN_TARGET", 0.060)
    ratio_std_target = _env_float("RATIO_STD_TARGET", 0.015)
    ...
    neighbor_collab_coef_start = _env_float("NEIGHBOR_COLLAB_COEF_START", 0.10)
    neighbor_collab_coef_end = _env_float("NEIGHBOR_COLLAB_COEF_END", 0.30)
    neighbor_prob_target = _env_float("NEIGHBOR_PROB_TARGET", 0.15)
    local_dominance_coef_start = _env_float("LOCAL_DOMINANCE_COEF_START", 0.05)
    local_dominance_coef_end = _env_float("LOCAL_DOMINANCE_COEF_END", 0.15)
    ...
    self_imitation_coef_start = _env_float("SELF_IMITATION_COEF_START", 0.30)
    self_imitation_coef_end = _env_float("SELF_IMITATION_COEF_END", 0.80)

    # v7 core fix: distill the exact one-step refined low-cost ratio back into
    # the Transformer ratio branch. First-stage data showed that ActorOnly ratio
    # stays around 0.14 while Refine16 improves it to about 0.055; without this
    # term, Stage-2 only benefits from online refine but the learned ratio head
    # itself remains biased high.
    refined_ratio_distill = _env_bool("REFINED_RATIO_DISTILL", True)
    refined_ratio_bc_coef_start = _env_float("REFINED_RATIO_BC_COEF_START", 0.80)
    refined_ratio_bc_coef_end = _env_float("REFINED_RATIO_BC_COEF_END", 0.25)

    refined_sched_distill = _env_bool("REFINED_SCHED_DISTILL", True)
    refined_sched_ce_coef_start = _env_float("REFINED_SCHED_CE_COEF_START", 0.60)
    refined_sched_ce_coef_end = _env_float("REFINED_SCHED_CE_COEF_END", 0.20)
    schedule_entropy_coef_start = _env_float("SCHEDULE_ENTROPY_COEF_START", 0.010)
    schedule_entropy_coef_end = _env_float("SCHEDULE_ENTROPY_COEF_END", 0.000)
    ...
    model_refine_max_tasks = _env_int("MODEL_REFINE_MAX_TASKS", 16)
    ...
    default_run_name = (
        f"proposed_full_stage2_v8_transformer_sched_k{K}_seed{base_seed}"
    ...
    # v8 results are stored in an isolated subdirectory.
    # Default:
    #   results/convergence_training/v8_transformer_sched/<run_name>/
    result_root = _env_str(
        "RESULT_ROOT",
        os.path.join("results", "convergence_training", "v8_transformer_sched"),
    )
    ...
    eval_actoronly_log_path = os.path.join(result_dir, "eval_actoronly_log.csv")
    ...
    with open(train_log_path, "w", newline="", encoding="utf-8") as f:
        ...
        writer.writerow([
            ...
            "avg_ratio_bc_loss",
            "avg_refined_ratio_bc_loss",
            "avg_refined_sched_ce_loss",
            "avg_refined_sched_acc",
            "avg_sched_entropy",
            "avg_executed_ratio_mean",
            "avg_ratio_target_delta",
            "avg_ratio_reg_loss",
    ...
    with open(eval_actoronly_log_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow([
            "eval_episode",
            "episode_reward",
    ...
    # v8: MLP actor is used for mobility only. Scheduling is generated by
    # TransformerScheduleHead, so the Stage-1 raw scheduling logits are not anchored here.
    ...
    # v7b: choose checkpoints by actor-only validation by default. Refine16 is
    # too strong and hides whether the learned policy itself has improved.
    best_select_mode = _env_str("BEST_SELECT_MODE", "actoronly").strip().lower()
    ...
    print(f"  refined_ratio_distill={refined_ratio_distill}, refined_ratio_bc_coef={refined_ratio_bc_coef_start}->{refined_ratio_bc_coef_end}")
    print(f"  refined_sched_distill={refined_sched_distill}, refined_sched_ce_coef={refined_sched_ce_coef_start}->{refined_sched_ce_coef_end}, entropy={schedule_entropy_coef_start}->{schedule_entropy_coef_end}")
    ...
    if num_layers == 0:
        print("  [ABLATION] NUM_LAYERS=0: Proposed w/o Transformer self-attention.")
    print(f"  rollback_patience={rollback_patience}, tolerance={rollback_tolerance}, lr_decay={rollback_lr_decay}")
    print(f"  best_select_mode={best_select_mode}")
    ...
    for ep in range(num_episodes):
        ...
        # episode_refined_ratio_bc_loss = 0.0
        # episode_executed_ratio_mean = 0.0
        # episode_ratio_target_delta = 0.0
        episode_refined_ratio_bc_loss = 0.0
        episode_refined_sched_ce_loss = 0.0
        episode_refined_sched_acc = 0.0
        episode_sched_entropy = 0.0
        episode_executed_ratio_mean = 0.0
        episode_ratio_target_delta = 0.0
        ...
        while not done:
            ...
            if (
                ...
                refined_ratio_bc_coef_t = linear_schedule_value(
                    refined_ratio_bc_coef_start, refined_ratio_bc_coef_end, coef_progress
                )
                refined_sched_ce_coef_t = linear_schedule_value(
                    refined_sched_ce_coef_start, refined_sched_ce_coef_end, coef_progress
                )
                schedule_entropy_coef_t = linear_schedule_value(
                    schedule_entropy_coef_start, schedule_entropy_coef_end, coef_progress
                )
                ...
                ratio_pred, schedule_logits, schedule_mask, candidate_j_t, sched_beta_soft = forward_v8_highlevel_branches(
                    policy=policy,
                    state=state,
                    access_assoc=access_assoc,
                    use_hard_min=False,
                ...
                surrogate_full_action = build_surrogate_full_action_tensor_v8(
                    state=state,
                    raw_action_pred=raw_pred,
                    ratio_pred=ratio_pred_for_action,
                    sched_beta_soft=sched_beta_soft,
                ...
                executed_offload_ratio_t = torch.tensor(
                    np.asarray(action["offload_ratio"], dtype=np.float32),
                    dtype=torch.float32,
                    device=device,
                )
                # The executed action is the safe + model-refined action. This
                # target is detached by construction and represents the low-cost
                # ratio found by the exact one-step model search for the current state.
                refined_ratio_bc_loss = mse_loss(ratio_pred, executed_offload_ratio_t)
                with torch.no_grad():
                    executed_ratio_mean_t = executed_offload_ratio_t.mean()
                    ratio_target_delta_t = torch.mean(torch.abs(ratio_pred.detach() - executed_offload_ratio_t))

                refined_sched_ce_loss, refined_sched_acc_t, sched_entropy_t = refined_schedule_distillation_loss(
                    schedule_logits=schedule_logits,
                    schedule_mask=schedule_mask,
                    candidate_j=candidate_j_t,
                    refined_sched_beta_np=np.asarray(action["sched_beta"], dtype=np.float32),
                    access_assoc=access_assoc,
                )
                ...
                neighbor_collab_loss = compute_v8_schedule_neighbor_collaboration_loss(
                    schedule_logits=schedule_logits,
                    schedule_mask=schedule_mask,
                    candidate_j=candidate_j_t,
                    state=state,
                ...
                local_dominance_loss = compute_v8_schedule_local_dominance_margin_loss(
                    schedule_logits=schedule_logits,
                    schedule_mask=schedule_mask,
                    candidate_j=candidate_j_t,
                    state=state,
                ...
                total_actor_loss = (
                    ...
                    + (refined_ratio_bc_coef_t * refined_ratio_bc_loss if refined_ratio_distill else 0.0)
                    + (refined_sched_ce_coef_t * refined_sched_ce_loss if refined_sched_distill else 0.0)
                    - schedule_entropy_coef_t * sched_entropy_t
                ...
                soft_update_policy_v8(target_policy, policy, tau=tau)
                ...
                # episode_refined_ratio_bc_loss += float(refined_ratio_bc_loss.item())
                # episode_executed_ratio_mean += float(executed_ratio_mean_t.item())
                
                episode_actor_policy_loss += float(actor_policy_loss.item())
                episode_move_sched_bc_loss += float(actor_move_sched_bc_loss.item())
                episode_ratio_bc_loss += float(ratio_bc_loss.item())
                episode_refined_ratio_bc_loss += float(refined_ratio_bc_loss.item())
                episode_refined_sched_ce_loss += float(refined_sched_ce_loss.item())
                episode_refined_sched_acc += float(refined_sched_acc_t.item())
                episode_sched_entropy += float(sched_entropy_t.item())
                episode_executed_ratio_mean += float(executed_ratio_mean_t.item())
                episode_ratio_target_delta += float(ratio_target_delta_t.item())
                ...
                if step_count < 2:
                    with torch.no_grad():
                        ...
                        print("  refined_ratio_target[:5] =", executed_offload_ratio_t.detach().cpu().numpy()[:5])
                        print("  ratio_target_delta =", float(ratio_target_delta_t.item()))
                        print("  refined_ratio_bc_loss =", float(refined_ratio_bc_loss.item()))
                        print("  refined_sched_ce_loss =", float(refined_sched_ce_loss.item()))
                        print("  refined_sched_acc =", float(refined_sched_acc_t.item()))
        ...
        avg_refined_ratio_bc_loss_value = (
            episode_refined_ratio_bc_loss / actor_update_count if actor_update_count > 0 else np.nan
        )
        avg_refined_sched_ce_loss_value = (
            episode_refined_sched_ce_loss / actor_update_count if actor_update_count > 0 else np.nan
        )
        avg_refined_sched_acc_value = (
            episode_refined_sched_acc / actor_update_count if actor_update_count > 0 else np.nan
        )
        avg_sched_entropy_value = (
            episode_sched_entropy / actor_update_count if actor_update_count > 0 else np.nan
        )
        avg_executed_ratio_mean_value = (
            episode_executed_ratio_mean / actor_update_count if actor_update_count > 0 else np.nan
        )
        avg_ratio_target_delta_value = (
            episode_ratio_target_delta / actor_update_count if actor_update_count > 0 else np.nan
        )
        ...
        print("avg_refined_ratio_bc_loss:", avg_refined_ratio_bc_loss_value)
        print("avg_refined_sched_ce_loss:", avg_refined_sched_ce_loss_value)
        print("avg_refined_sched_acc:", avg_refined_sched_acc_value)
        print("avg_sched_entropy:", avg_sched_entropy_value)
        print("avg_executed_ratio_mean:", avg_executed_ratio_mean_value)
        print("avg_ratio_target_delta:", avg_ratio_target_delta_value)
        ...
        if (ep + 1) % eval_every == 0 or ep == 0:
            ...
            with policy_eval_mode(policy):
                # Main deployment evaluation: safe mobility + full refine16.
                eval_result = evaluate_policy_with_refine_override(
                    env=eval_env,
                    policy=policy,
                    seed=eval_seed,
                    enable_refine=True,
                ...
                # Learned-policy evaluation: safe mobility only, no model refine.
                # This is the correct validation signal for whether Stage-2
                # ratio distillation actually improves the network body.
                actoronly_env = MultiUavMecEnv(
                    M=M,
                    K=K,
                    episode_length=env.episode_length,
                    cpu_mode=env.cpu_mode,
                ...
                eval_actoronly_result = evaluate_policy_with_refine_override(
                    env=actoronly_env,
                    policy=policy,
                    seed=eval_seed,
                    enable_refine=False,
            ...
            print("[ActorOnly Eval] system_cost:", eval_actoronly_result.get("system_cost", -eval_actoronly_result["episode_reward"]))
            print("[ActorOnly Eval] avg_delay:", eval_actoronly_result["avg_delay"])
            print("[ActorOnly Eval] feasible_ratio:", eval_actoronly_result["feasible_ratio"])
            print("[ActorOnly Eval] ratio_mean:", eval_actoronly_result.get("ratio_mean", float("nan")))
            ...
            with open(eval_actoronly_log_path, "a", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)
                writer.writerow([
                    ep,
                    eval_actoronly_result["episode_reward"],
                    eval_actoronly_result.get("system_cost", -eval_actoronly_result["episode_reward"]),
                    eval_actoronly_result["avg_delay"],
                    eval_actoronly_result["avg_energy"],
                    eval_actoronly_result["avg_deadline_violation"],
                    eval_actoronly_result["feasible_ratio"],
                    eval_actoronly_result["num_steps"],
                    eval_actoronly_result["avg_ratio_violation"],
                    eval_actoronly_result["avg_assoc_violation"],
                    eval_actoronly_result["avg_schedule_violation"],
                    eval_actoronly_result["avg_candidate_violation"],
                    eval_actoronly_result["avg_bw_violation"],
                    eval_actoronly_result["avg_cpu_violation"],
                    eval_actoronly_result["avg_rate_violation"],
                    eval_actoronly_result["avg_nan_count"],
                    eval_actoronly_result["avg_move_violation"],
                    eval_actoronly_result["avg_boundary_violation"],
                    eval_actoronly_result["avg_collision_violation"],
                    eval_actoronly_result["avg_battery_violation"],
                    eval_actoronly_result.get("ratio_mean", float("nan")),
                    eval_actoronly_result.get("ratio_std", float("nan")),
                    eval_actoronly_result.get("ratio_min", float("nan")),
                    eval_actoronly_result.get("ratio_max", float("nan")),
                    eval_actoronly_result.get("local_exec_ratio", float("nan")),
                    eval_actoronly_result.get("neighbor_exec_ratio", float("nan")),
                ])

            selected_eval_result = eval_actoronly_result if best_select_mode == "actoronly" else eval_result
            selected_eval_reward = selected_eval_result["episode_reward"]

            if ep >= eval_best_start_episode and selected_eval_reward > best_eval_reward:
                best_eval_reward = selected_eval_reward
                best_policy_state = copy.deepcopy(policy)
                eval_bad_count = 0
                print(f"[BEST:{best_select_mode}] updated best eval reward to {best_eval_reward:.6f} at episode {ep}")
                ...
                if selected_eval_reward < best_eval_reward - rollback_tolerance:
                    eval_bad_count += 1
                else:
                    eval_bad_count = 0
    ...
    # -------------------------------------------------
    # Save final stage-2 checkpoints as an additional diagnostic target.
    # You can evaluate this with prefix: {CKPT_PREFIX}_final
    # -------------------------------------------------
#  # -------------------------------------------------
#     # Save final / best stage-2 checkpoints
#     # -------------------------------------------------
#     os.makedirs("checkpoints", exist_ok=True)
...
#     # Save final stage-2 checkpoints as an additional diagnostic target.
#     # You can evaluate this with prefix: {CKPT_PREFIX}_final
#     save_policy_components(policy, ckpt_prefix, "final")
...
#     print(f"  {eval_actoronly_log_path}")
    # The saved checkpoints below always use best_policy_state, so they never
    # correspond to a degraded last-episode actor.
    ...
    save_meta = {
        "version": "v8_transformer_sched",
        "run_name": run_name,
        "ckpt_prefix": ckpt_prefix,
        "result_dir": result_dir,
        "train_log_path": train_log_path,
        "eval_log_path": eval_log_path,
        "eval_actoronly_log_path": eval_actoronly_log_path,
        ...
        "strict_stage1": strict_stage1,
        "embed_dim": embed_dim,
        "num_heads": num_heads,
        "ff_hidden_dim": ff_hidden_dim,
        "num_layers": num_layers,
        "encoder_ablation": "meanpool_no_self_attention" if num_layers == 0 else "transformer",
        "best_select_mode": best_select_mode,
        "best_eval_reward": best_eval_reward,
        "critic_only_episodes": critic_only_episodes,
        ...
        "model_refine_enable": model_refine_enable,
        "model_refine_max_tasks": model_refine_max_tasks,
        "model_refine_ratio": model_refine_ratio,
        "model_refine_sched": model_refine_sched,
        "refined_ratio_distill": refined_ratio_distill,
        "refined_sched_distill": refined_sched_distill,
        "actor_policy_coef_start": actor_policy_coef_start,
        "actor_policy_coef_end": actor_policy_coef_end,
        "actor_move_sched_bc_coef_start": actor_move_sched_bc_coef_start,
        "actor_move_sched_bc_coef_end": actor_move_sched_bc_coef_end,
        "ratio_bc_coef_start": ratio_bc_coef_start,
        "ratio_bc_coef_end": ratio_bc_coef_end,
        "ratio_reg_coef_start": ratio_reg_coef_start,
        "ratio_reg_coef_end": ratio_reg_coef_end,
        "ratio_floor_target": ratio_floor_target,
        "ratio_mean_target": ratio_mean_target,
        "ratio_std_target": ratio_std_target,
        "refined_ratio_bc_coef_start": refined_ratio_bc_coef_start,
        "refined_ratio_bc_coef_end": refined_ratio_bc_coef_end,
        "refined_sched_ce_coef_start": refined_sched_ce_coef_start,
        "refined_sched_ce_coef_end": refined_sched_ce_coef_end,
        "schedule_entropy_coef_start": schedule_entropy_coef_start,
        "schedule_entropy_coef_end": schedule_entropy_coef_end,
        "neighbor_collab_coef_start": neighbor_collab_coef_start,
        "neighbor_collab_coef_end": neighbor_collab_coef_end,
        "neighbor_prob_target": neighbor_prob_target,
        "local_dominance_coef_start": local_dominance_coef_start,
        "local_dominance_coef_end": local_dominance_coef_end,
        "local_dominance_margin": local_dominance_margin,
        "self_imitation_coef_start": self_imitation_coef_start,
        "self_imitation_coef_end": self_imitation_coef_end,
        "elite_capacity": elite_capacity,
        "elite_batch_size": elite_batch_size,
        "elite_min_size": elite_min_size,
    ...
    # 1) Save final policy-side components.
    # Evaluation prefix for this one is: {ckpt_prefix}_final
    save_policy_components(policy, ckpt_prefix, "final")

    # 2) Save final critic-side components.
    torch.save(
        critic.state_dict(),
        f"checkpoints/{ckpt_prefix}_final_critic.pth",
    )
    torch.save(
        target_critic.state_dict(),
        f"checkpoints/{ckpt_prefix}_final_target_critic.pth",
    )

    # 3) Save best policy-side components.
    # Evaluation prefix for this one is exactly: {ckpt_prefix}
    save_policy_components(best_policy_state, ckpt_prefix, "best")

    # 4) Save critic used at the end of training.
    # Keep this old name for compatibility with existing scripts.
    torch.save(
        critic.state_dict(),
        f"checkpoints/{ckpt_prefix}_critic.pth",
    )
    torch.save(
        target_critic.state_dict(),
        f"checkpoints/{ckpt_prefix}_target_critic.pth",
    )

    # 5) Save complete training state for reproducibility / resume.
    save_training_state(
        path=f"checkpoints/{ckpt_prefix}_training_state.pth",
        policy=policy,
    ...
    save_training_state(
        path=f"checkpoints/{ckpt_prefix}_best_training_state.pth",
        ...
        actor_opt=actor_opt,
        critic_opt=critic_opt,
        meta={**save_meta, "state_kind": "best_policy_state"},
    )

    print("\nFull proposed Stage-2 v8 training finished successfully.")
    print("Saved best policy checkpoints:")
    print(f"  checkpoints/{ckpt_prefix}_best_actor.pth")
    print(f"  checkpoints/{ckpt_prefix}_best_encoder.pth")
    print(f"  checkpoints/{ckpt_prefix}_best_fusion.pth")
    print(f"  checkpoints/{ckpt_prefix}_best_ratio_head.pth")
    print(f"  checkpoints/{ckpt_prefix}_best_schedule_head.pth")
    print(f"  checkpoints/{ckpt_prefix}_critic.pth")
    print(f"  checkpoints/{ckpt_prefix}_target_critic.pth")
    print("Saved final policy checkpoints:")
    print(f"  checkpoints/{ckpt_prefix}_final_actor.pth")
    print(f"  checkpoints/{ckpt_prefix}_final_encoder.pth")
    print(f"  checkpoints/{ckpt_prefix}_final_fusion.pth")
    print(f"  checkpoints/{ckpt_prefix}_final_ratio_head.pth")
    print(f"  checkpoints/{ckpt_prefix}_final_schedule_head.pth")
    print(f"  checkpoints/{ckpt_prefix}_final_critic.pth")
    print(f"  checkpoints/{ckpt_prefix}_final_target_critic.pth")
    print("Saved full training states:")
    print(f"  checkpoints/{ckpt_prefix}_training_state.pth")
    print(f"  checkpoints/{ckpt_prefix}_best_training_state.pth")
    print("Logs:")
    print(f"  {train_log_path}")
    print(f"  {eval_log_path}")
    print(f"  {eval_actoronly_log_path}")
