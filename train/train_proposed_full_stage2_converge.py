...
# Stage-2 收敛训练版（从 safemobility_ratio_reg 版本整理）：
...
def main():
    ...
    actor_opt = optim.Adam(
        [
            ...
            {"params": ratio_branch_params, "lr": 3e-4},
        ]
    )
    ...
    actor_move_sched_bc_coef = _env_float("ACTOR_MOVE_SCHED_BC_COEF", 0.7)
    ratio_bc_coef = _env_float("RATIO_BC_COEF", 0.01)

    # Proposed ratio-head offloading regularization.
    # Start at 1.0 because the previous 0.5 setting did not pull ratio_pred upward.
    ratio_reg_coef = _env_float("RATIO_REG_COEF", 1.0)
    ratio_floor_target = _env_float("RATIO_FLOOR_TARGET", 0.15)
    ratio_mean_target = _env_float("RATIO_MEAN_TARGET", 0.20)
    ratio_std_target = _env_float("RATIO_STD_TARGET", 0.06)
    ...
    default_run_name = (
        ...
        f"_cw{critic_only_episodes}_rr{ratio_reg_coef:g}_{timestamp}"
    )
    ...
    # Do not allow a pre-actor-update evaluation to become the saved best checkpoint.
    # Otherwise the saved model may only reflect Stage-1 warm start rather than Stage-2 ratio regularization.
    eval_best_start_episode = critic_only_episodes + 20
    ...
    print(f"  actor_policy_coef={actor_policy_coef}, actor_move_sched_bc_coef={actor_move_sched_bc_coef}, ratio_bc_coef={ratio_bc_coef}")
    print(f"  ratio_reg_coef={ratio_reg_coef}, floor/mean/std targets={ratio_floor_target}/{ratio_mean_target}/{ratio_std_target}")
